...
- 🔲 **외각선 모드 (`OutlineVisualizer`)**: 파도 모드를 기반으로, 가장자리까지의 채움을 제외합니다.
...
- **실시간 분류 기능**: 소프트웨어에 내장된 `SoundClassifier` 모델이 배경음(Ambient), 음성(Speech), 위협음(Danger) 등의 오디오 이벤트를 감지하고 라벨링합니다.
...
본 프로젝트에서 사용된 서드파티 라이브러리 및 AI 모델의 출처와 라이선스 정보는 [NOTICE.md](NOTICE.md) 파일을 참조해 주세요.
