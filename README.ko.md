...
- **커스터마이징**: PC 성능 저하를 최소화하면서 스트리밍 또는 듀얼 모니터 환경에 커스텀 오버레이를 제공합니다.
...
### 다양한 시각화 모드
- **파도 모드 (`WaveVisualizer`)**: 소리의 강도에 따라 스케일이 조정되는 오디오 파도가 표현됩니다.
- **원형 모드 (`CircleRippleVisualizer`)**: 소리가 화면 중심을 기준으로 화면 바깥을 향해 원형으로 파도가 표현됩니다. 
- **패드 모드 (`PadVisualizer`)**: 2.0 / 5.1 / 7.1 공간 그리드 방향에 따라 고정된 지점에 사운드가 표현됩니다.
- **외곽선 모드 (`OutlineVisualizer`)**: 파도 모드를 기반으로, 가장자리까지의 채움을 제외합니다.

### 실시간 오버레이 에디터 (기본 F4 키)
- **드래그 앤 리사이즈**: **F4** 키를 눌러 에디터 모드를 활성화할 수 있습니다. 화면의 가이드라인 경계를 직접 드래그하여 그래픽의 렌더링 한계를 실시간으로 조정합니다.
- **온스크린 제어 패널**: 실시간 오버레이 제어 패널을 통해 색상, 민감도, 속도, 발광 효과 및 AI 음성 감지 라벨 등 모든 설정을 동적으로 변경할 수 있습니다.

### 매끄러운 단축키 제어
- 사용자가 설정한 단축키(**F2**: 사운드 모드 변경, **F3**: 시각화 모드 변경, **F4**: 오버레이 에디터 토글)를 지원하여, 전체 화면 게임을 최소화하지 않고도 모드를 즉시 전환할 수 있습니다.

### 고급 다중 채널 오디오 지원
- **하드웨어 인식 설계**: **2.0 스테레오**, **5.1 서라운드**, **7.1 서라운드** 채널을 자동으로 감지하고 구성합니다. 필요에 따라 사용자가 채널을 설정하는 것도 가능합니다.
- **가상 7.1 서라운드 지원**: **VB-CABLE**과 같은 가상 오디오 툴 설정을 통해 스테레오 환경에서도 7.1 서라운드의 입체감 있는 시각화 오버레이를 경험할 수 있습니다.

### AI 사운드 분류 (ONNX & YAMNet)
- **실시간 분류 기능**: YAMNet을 기반으로 파인튜닝(전이 학습)된 맞춤형 모델(`three_class_score_head`, `gunshot_booster`)이 내장된 `SoundClassifier`를 통해 배경음(Ambient), 음성(Speech), 위협음(Danger) 및 총소리 등을 빠르고 정확하게 감지하고 라벨링합니다.
- **시각 피드백**: 각 감지 카테고리마다 사용자 지정 UI 색상 및 불투명도(Opacity) 독립 설정을 할당하여 더욱 직관적으로 인지할 수 있습니다.

### 다국어 지원 (8개 국어)
- **한국어**, 영어, 일본어, 중국어, 스페인어, 프랑스어, 독일어, 러시아어를 지원합니다.
...
└── tools/
    └── transfer_learning/  # 자체 커스텀 ONNX 모델 생성 및 전이 학습을 위한 스크립트 구역
```
...
## 소프트웨어 라이선스 정책
본 프로젝트는 AGPL v3 라이선스에 따라 자유롭게 배포되고 수정될수 있습니다 - 자세한 내용은 코어의 LICENSE 파일을 참조해 주세요.

본 프로젝트에서 사용된 서드파티 라이브러리 및 AI 모델의 출처와 라이선스 정보는 [THIRD_PARTY.md](THIRD_PARTY.md) 파일을 참조해 주세요.
