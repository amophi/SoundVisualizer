...
- **F2 / F3 단축키**: AI 사운드 모드 및 시각화 모드 실시간 변경.
...
본 프로젝트는 대학교 공개SW프로젝트 교과목의 일환으로 아래 팀원들의 기여를 통해 완성되었습니다.

| 이름 | 역할 및 주요 담당 업무 |
| :---: | :--- |
| **김도환** (팀장) | 프로젝트 총괄, WPF 투명 오버레이 렌더링 코어 개발, UI/UX 설계 및 다국어 지원 구현, 최종보고서 취합 |
| **김성진** (팀원) | WASAPI 저지연 오디오 캡처 엔진 및 DSP 파이프라인 구축, 고속 연산 스레드 최적화, 프레젠테이션 |
| **박가람** (팀원) | AI 실시간 소리 분류 파이프라인 총괄, YAMNet 3-Class 모델 전이 학습 및 Gunshot Booster 개발 |
| **양수현** (팀원) | UI/UX 방향성 리서치, AI 학습용 추가 오디오 데이터셋 수집, 청각장애인 대상 접근성 설문조사 진행 |
